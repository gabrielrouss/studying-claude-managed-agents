...
def main():
    ...
    print(f"    Sub-agentes:    {len(orchestrator.callable_agents)} registrados")
        ...
        for event in stream:
            match event.type:
                ...
                case "session.thread_created":
                    # Novo sub-agente iniciou uma thread
                ...
                case "agent.message":
                    for block in event.content:
                        if hasattr(block, "text"):
                            # Identificar se e do orchestrator ou sub-agente
                            text = block.text
                            print(f"\n[DIRECTOR] {text}")
                ...
                case "agent.tool_use":
                    total_tools += 1
                    name = event.name
                    # Em multi-agent, as tool_use dos sub-agentes NAO
                    # aparecem no stream principal - apenas no stream
                    # de cada thread. Aqui so vemos tools do orchestrator.
                    print(f"\n🔧 [TOOL] {name}")
    ...
    # =========================================================================
    # CONCEITO: Apos a execucao, voce pode listar todas as threads
    # e inspecionar o historico de cada uma individualmente.
    print("\n" + "=" * 60)
    ...
    try:
        ...
        for thread in threads:
            print(
                f"  [{thread.agent_name}] Status: {thread.status}"
            )
    ...
    for a in [research_agent, writer_agent, adapter_agent, orchestrator]:
        client.beta.agents.archive(a.id)
        print(f"  Agent {a.name} arquivado")
    ...
    print("A Content Factory esta completa! 🎉")
